...
base_url = ("https://fred.stlouisfed.org/graph/fredgraph.csv?"
            ...
            "id=BAMLH0A0HYM2&scale=left&cosd=2019-10-10&coed=2024-10-10&line_color=%234572a7&"
...
data['median_3650'] = data['BAMLH0A0HYM2_filled'].rolling(window=3650, min_periods=1).median()
...
data['rolling_avg_7'] = data['BAMLH0A0HYM2_filled'].rolling(window=7, min_periods=1).mean()
...
data['status'] = data.apply(lambda row: calculate_status(row['rolling_avg_7'], row['future_avg']), axis=1)
...
data['position'] = data.apply(lambda row: calculate_position(row['BAMLH0A0HYM2_filled'], row['median_3650']), axis=1)
...
first_occurrence = data.iloc[first_occurrence_index] if first_occurrence_index >= 0 else data.iloc[-1]

# Step 3: Find the preceding status before the block of latest_status
preceding_status = data.loc[first_occurrence_index + 1:, 'final_status'].replace("", float("nan")).dropna().iloc[0] if first_occurrence_index + 1 < len(data) else "N/A"

# Print the current status, date since it started, and preceding status
print(f'Current status: "{latest_status}", since: "{first_occurrence["Date"].date()}", before: "{preceding_status}"')
